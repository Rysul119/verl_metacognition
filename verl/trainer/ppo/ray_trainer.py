...
from verl.trainer.ppo.reward import compute_reward, compute_reward_async, compute_reward_reasoning, compute_reward_metacognitive
...
from verl.utils.dataset.rl_dataset import HoldoutWrapped
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._create_dataloader(train_dataset, val_dataset, holdout_dataset, collate_fn, train_sampler)
    ...
    def _create_dataloader(self, train_dataset, val_dataset, holdout_dataset, collate_fn, train_sampler):
        """
        ...
        if self.config.reward_model.metacognition.enabled:
            self.holdout_dataset = create_rl_dataset(
                self.config.data.holdout_files, self.config.data, self.tokenizer, self.processor
            )
            #self.holdout_dataset = HoldoutWrapped(holdout_dataset, label_key="label")
            self.holdout_dataloader = StatefulDataLoader(
                dataset=self.holdout_dataset,
                batch_size=self.config.data.train_batch_size, # to load the whole batch of data
                shuffle=False,
                num_workers=self.config.data.get("dataloader_num_workers", 8),
                collate_fn=collate_fn,   # reuse the same collate_fn
                drop_last=False,
                pin_memory=False,
        )
    ...
    def _validate(self):
        ...
        data_src2var2metric2val = process_validation_metrics(data_sources[: len(sample_inputs)], sample_inputs, reward_extra_infos_dict)
        metric_dict = {}
